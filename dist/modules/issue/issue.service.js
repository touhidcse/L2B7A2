...
const getAllIssuesFromDB = async (query) => {
    ...
    if (sort && !allowedSort.includes(sort)) {
        throw new Error("Invalid sort value. Allowed values: newest, oldest");
    }
    if (type && !allowedTypes.includes(type)) {
        throw new Error("Invalid type value. Allowed values: bug, feature_request");
    }
    if (status && !allowedStatuses.includes(status)) {
        throw new Error("Invalid status value. Allowed values: open, in_progress, resolved");
    }
    const conditions = [];
    const values = [];
    // Type filter
...
// 6. Update Issue
const updateIssueIntoDB = async (payload, issueId, userId, role) => {
    const issueResult = await pool.query(`
    SELECT *
    FROM issues
    WHERE id = $1
    ...
    if (role === USER_ROLE.contributor) {
        if (issue.reporter_id !== userId) {
            throw new Error("Contributor can update only own issue");
        }
        if (issue.status !== ISSUE_STATUS.open) {
            throw new Error("Contributor can update only open issues");
        }
    }
